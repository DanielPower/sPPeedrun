...
async function requestAppToken(): Promise<string> {
  ...
  if (!res.ok) {
    throw new Error(
      `App token request failed: ${res.status} ${await res.text()}`,
    )
  }
...
export async function fetchScores(
  ...
  if (!res.ok) {
    throw new Error(
      `Failed to fetch /scores: ${res.status} ${await res.text()}`,
    )
  }
...
export async function fetchBeatmap(id: number): Promise<OsuBeatmapFull | null> {
  ...
  if (!res.ok) {
    throw new Error(
      `Failed to fetch /beatmaps/${id}: ${res.status} ${await res.text()}`,
    )
  }
